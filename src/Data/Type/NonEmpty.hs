...
-- instance (Linear c a, Linear c (T as)) => Linear c (NETup (a ':| as)) where
--     NET x xs .+. NET y ys = NET (x .+. y) (xs .+. ys)
--     zeroL               = NET zeroL zeroL
...
-- instance (Linear s a, Linear s (T as), Mutable m (NETup (a ':| as)))
--         => LinearInPlace m s (NETup (a ':| as))
