{-# LANGUAGE AllowAmbiguousTypes    #-}
{-# LANGUAGE DataKinds              #-}
{-# LANGUAGE FlexibleContexts       #-}
{-# LANGUAGE FlexibleInstances      #-}
{-# LANGUAGE FunctionalDependencies #-}
{-# LANGUAGE GADTs                  #-}
{-# LANGUAGE LambdaCase             #-}
{-# LANGUAGE MultiParamTypeClasses  #-}
{-# LANGUAGE PolyKinds              #-}
{-# LANGUAGE RankNTypes             #-}
{-# LANGUAGE ScopedTypeVariables    #-}
{-# LANGUAGE TupleSections          #-}
{-# LANGUAGE TypeApplications       #-}
{-# LANGUAGE TypeFamilies           #-}
{-# LANGUAGE TypeFamilyDependencies #-}
{-# LANGUAGE TypeInType             #-}
{-# LANGUAGE TypeOperators          #-}
{-# LANGUAGE TypeSynonymInstances   #-}
{-# LANGUAGE UndecidableInstances   #-}

module Learn.Neural
  where

-- import           Type.Family.List
-- import           Type.Family.Maybe
import           Control.Monad.Primitive
import           Control.Monad.ST
import           Data.Bifunctor
import           Data.Kind
import           Data.Singletons.Prelude
import           Data.Type.Index
import           Data.Type.Option
import           GHC.TypeLits
import           Numeric.BLAS
import           Numeric.Backprop
import           Numeric.Backprop.Iso    (Iso', iso, from, tup1)
import           Numeric.Backprop.Op
import           Statistics.Distribution
import           System.Random.MWC
import           Type.Family.Bool

type family MaybeToList (a :: Maybe k) = (b :: [k]) | b -> a where
    MaybeToList ('Just a ) = '[a]
    MaybeToList 'Nothing   = '[]

class Component c where
    type CParam c (b :: BShape Nat -> Type) (i :: BShape Nat) (o :: BShape Nat) = (p :: Type      ) | p -> c
    type CState c (b :: BShape Nat -> Type) (i :: BShape Nat) (o :: BShape Nat) = (s :: Maybe Type) | s -> c

    runComponent
        :: forall s b i o. ()
        => OpB s (b i ': CParam c b i o ': MaybeToList (CState c b i o))
                 (b o ': MaybeToList (CState c b i o))
    initComponent
        :: ContGen d
        => Sing i
        -> Sing o
        -> d
        -> Gen (PrimState m)
        -> m (Tuple (CParam c b i o ': MaybeToList (CState c b i o)))

data HasState = NoState
              | SomeState

data Layer :: Type -> (BShape Nat -> Type) -> BShape Nat -> BShape Nat -> Type where
    LPure  :: (CState c b i o ~ 'Nothing) => CParam c b i o -> Layer c b i o
    LState :: (CState c b i o ~ 'Just s ) => CParam c b i o -> s -> Layer c b i o

instance Num (Layer c b i o)

layerOp
    :: forall s c b i o. Component c
    => OpB s '[ b i, Layer c b i o ] '[ b o, Layer c b i o ]
layerOp = OpM $ \(I x :< I l :< Ø) -> case l of
    LPure p -> do
      (I y :< Ø, gF) <- runOpM' (runComponent @c @s @b @i @o) (x ::< p ::< Ø)
      let gF' = fmap (\case I dX :< I dP :< Ø -> I dX :< I (LPure dP) :< Ø)
              . gF
              . (\case dY :< _ :< Ø -> dY :< Ø)
      return (y ::< LPure p ::< Ø, gF')
    LState p s -> do
      (I y :< I s' :< Ø, gF) <- runOpM' (runComponent @c @s @b @i @o) (x ::< p ::< s ::< Ø)
      let gF' = fmap (\case I dX :< I dP :< I dS :< Ø -> dX ::< LState dP dS ::< Ø)
              . gF
              . (\case dY :< Just (LState dP dS) :< Ø -> dY :< Just dS :< Ø
                       dY :< Nothing             :< Ø -> dY :< Nothing :< Ø
                )
      return (y ::< LState p s' ::< Ø, gF')

data LChain :: Type where
    (:~) :: BShape Nat -> Type -> LChain

data Network :: (BShape Nat -> Type) -> LChain -> [LChain] -> BShape Nat -> Type where
    NetExt :: Layer c b i o
           -> Network b (i ':~ c) '[] o
    (:&)   :: Component d
           => Layer c b i h
           -> Network b (h ':~ d) hs               o
           -> Network b (i ':~ c) ((h ':~ d) ': hs) o

networkOp
    :: forall s b i c hs o. (Component c)
    => OpB s '[ b i, Network b (i ':~ c) hs o ] '[ b o, Network b (i ':~ c) hs o ]
networkOp = OpM $ \(I x :< I n :< Ø) -> case n of
    NetExt l -> do
      (I y :< I l' :< Ø, gF) <- runOpM' (layerOp @s @c @b @i @o) (x ::< l ::< Ø)
      let gF' = fmap (\case I dX :< I dL :< Ø -> dX ::< NetExt dL ::< Ø)
              . gF
              . (\case dY :< Just (NetExt dL) :< Ø -> dY :< Just dL :< Ø
                       dY :< Nothing          :< Ø -> dY :< Nothing :< Ø
                )
      return (y ::< NetExt l' ::< Ø, gF')
    (l :: Layer c b i h) :& (n2 :: Network b (h ':~ d) js o) -> do
      (I y :< I l'  :< Ø, gF) <- runOpM' (layerOp @s @c @b @i) (x ::< l ::< Ø)
      (I z :< I n2' :< Ø, gF') <- runOpM' networkOp (y ::< n2 ::< Ø)
      let gF'' :: Prod Maybe '[ b o, Network b (i ':~ c) ((h ':~ d) ': js) o]
               -> ST s (Tuple '[ b i, Network b (i ':~ c) ((h ':~ d) ': js) o])
          gF'' = \case dZ :< Just (dL :& dN) :< Ø -> do
                         I dY :< I dN2 :< Ø <- gF' (dZ :< Just dN :< Ø)
                         I dX :< I dL0 :< Ø <- gF (Just dY :< Just dL :< Ø)
                         return $ dX ::< (dL0 :& dN2) ::< Ø
      return (z ::< (l' :& n2') ::< Ø, gF'')

